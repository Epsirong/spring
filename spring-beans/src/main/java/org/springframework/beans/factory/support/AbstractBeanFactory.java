...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
			...
			throws BeansException {
        //1. 剥离工厂引用前缀，如果name为alias，则获取对应的beanName
		String beanName = transformedBeanName(name);
		Object beanInstance;

		// Eagerly check singleton cache for manually registered singletons.
		// 2. 从缓存或实例工厂中，获取Bean对象
		// 尝试从缓存中获取目标对象，如果没获取到，则尝试获取半成品对象
		...
		if (sharedInstance != null && args == null) {
			...
			try {
				...
				if (mbd.isSingleton()) {
					//该getSingleton尝试创建目标对象，并注入其所依赖属性
	...
	protected Object getObjectForBeanInstance(
		...
		if (object == null) {
			...
			// containsBeanDefinition 检测 beanDefinitionMap 中也就是在所有已经加载的类中，检测是否定义 beanName
	...
	protected void registerDisposableBeanIfNecessary(String beanName, Object bean, RootBeanDefinition mbd) {
		...
		//@Component与@Bean一大区别：requiresDestruction
